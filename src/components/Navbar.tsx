  ...
  return (
    ...
    >
      <div className="c-navbar__container">
        <Link href="/" className="c-navbar__logo" onClick={closeMenu}>CODERON</Link>
        <nav className="c-navbar__desktop-nav">
          <ul>
            {navLinks.map((link) => {
              const isActive = pathname === link.href;
              return (<li key={link.href}><Link href={link.href} className={`c-navbar__link ${isActive ? 'is-active' : ''}`}>{link.label}</Link></li>);
            })}
          </ul>
        </nav>
